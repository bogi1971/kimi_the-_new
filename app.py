"""
Elite Bull Scanner Pro V8.1 - STABLE DOCUMENTATION EDITION
Dieser Code ist absichtlich so strukturiert, dass er selbsterklärend ist.
Änderungen werden inline dokumentiert mit # CHANGELOG: [Datum] - [Grund]
"""
...
# ==============================================================================
"""
DESIGN-PRINZIPIEN (unveränderlich):
1. STREAMLIT-LIMITATION: Keine echten Background-Tasks möglich. 
   Lösung: Autopilot via st.rerun() mit session_state-Timestamps.
   Alternative wäre external Cronjob + Datenbank, aber das übersteigt Scope.

2. YAHOO-RATE-LIMITING: Yahoo blockiert aggressive Scraper.
   Lösung: Exponential Backoff, Thread-Delays, Smart Caching.
   Keine Proxy-Rotation implementiert (nicht nötig für <200 Symbole).

3. CANDLESTICK-FILTER: Harte Filterung ist DESIGN-ENTSCHEIDUNG.
   Ziel: Qualität > Quantität. "Lieber 2 excellente als 20 moderate Setups."

4. API-KEY-ROTATION: Alpha Vantage (25/Tag) und Finnhub (60/Min) 
   werden mit Round-Robin + Rate-Limiting bedient.
"""
...
st.markdown("""
<style>
/* CHANGELOG: 2026-03-08 - Konsolidiertes, getestetes CSS */
.bull-card { 
...
class SourceType(str, Enum):
    """Quelle des Symbols im Scan-Universum."""
    WATCHLIST = "watchlist"
    GAINERS = "gainers"
...
class CandlestickPattern(str, Enum):
    """Unterstützte Candlestick-Muster für Entry-Bestätigung."""
    NONE = "none"
    HAMMER = "hammer"
...
class CandlestickSignal:
    """
    Repräsentiert ein erkanntes Candlestick-Muster mit Bewertung.
    
    DESIGN: Stärke 0-100, Quality-Levels für Filterung.
    Hard Filter erfordert strength >= 60 und quality in [excellent, good].
    """
...
@dataclass 
class ScanResult:
    """Vollständiges Ergebnis einer Symbol-Analyse."""
    symbol: str
    tier: int
    score: int
    price: float
...
st.set_page_config(
    layout="wide", 
    page_title="Elite Bull Scanner Pro V8.1", 
    page_icon="🕯️"
)

# FILTER-KONFIGURATION
# CHANGELOG: 2026-03-08 - Werte sind für US-Tech/Growth-Stacks optimiert.
# Bei Crypto/Forex müssen Pullback-Prozente angepasst werden (höhere Volatilität).
MIN_PULLBACK_PERCENT = 0.05
MAX_PULLBACK_PERCENT = 0.50
AUTO_REFRESH_INTERVAL = 3600  # Sekunden zwischen Movers-Updates
ALERT_COOLDOWN_MINUTES = 60
MIN_SCORE_THRESHOLD = 70
MIN_CANDLESTICK_STRENGTH = 60
REQUIRE_CANDLESTICK_CONFIRM = True
MAX_WATCHLIST_SIZE = 100

# API KEYS - Aus Streamlit Secrets
# CHANGELOG: 2026-03-08 - Fallback auf leere Werte bei fehlenden Secrets,
# damit App startet (aber mit eingeschränkter Funktionalität).
try:
...
# DEFAULT UNIVERSE
# CHANGELOG: 2026-03-08 - Fokus auf liquid US-Tech/Growth für Daytrading.
...
def init_session_state():
    """
    Initialisiert alle Session-State-Variablen mit Defaults.
    CHANGELOG: 2026-03-08 - Zentrale Initialisierung verhindert KeyErrors.
    """
    defaults = {
        ...
        'combined_universe': set(DEFAULT_WATCHLIST + [
            s for sublist in FALLBACK_MOVERS.values() for s in sublist
        ]),
        'movers_source': 'fallback',
        'hard_filter_active': True,
        'show_only_candlestick': True,
    }
...
class SmartCache:
    ...
    CHANGELOG: 2026-03-08 - Kein Max-Size-Limit implementiert, da 
    bei ~150 Symbolen × 4 Datenquellen Speicher vernachlässigbar.
    Für 1000+ Symbole wäre LRU-Caching nötig.
    """
    def __init__(self):
        self.cache = {}
        self.timestamps = {}
...
class RateLimiter:
    """
    Thread-sicheres Rate Limiting mit Sliding Window.
    
    CHANGELOG: 2026-03-08 - Verwendet threading.Lock für ThreadPool-Kompatibilität.
    """
...
class AlphaVantageManager:
    """
    Rotierendes Key-Management für Alpha Vantage (25 Calls/Tag/Key).
    
    CHANGELOG: 2026-03-08 - Implementiert Exhaustion-Tracking pro Key.
    Bei erschöpften Keys wird automatisch rotiert.
    """
...
def safe_requests_get(
    ...
    """Wrapper für robuste HTTP-Requests mit Exception-Handling."""
    try:
...
def get_market_clock() -> Dict[str, Any]:
    """
    Ermittelt Marktstatus (Open/Closed/Pre/After) in US/Eastern Zeit.
    
    CHANGELOG: 2026-03-08 - Berücksichtigt US-Feiertage 2026.
    """
...
def get_market_context() -> Dict[str, Any]:
    """
    Analysiert Markt-Kontext (SPY Trend, VIX Level).
    
    CHANGELOG: 2026-03-08 - Risk-Off bei SPY <-2% oder VIX >30.
    """
...
def fetch_yahoo_movers() -> Tuple[Dict[str, List[str]], str]:
    """
    Scrapt Yahoo Finance Gainers/Most Active.
    
    CHANGELOG: 2026-03-08 - Verwendet pd.read_html mit Fallback auf Default-Liste.
    Yahoo ändert häufig HTML-Struktur, daher robustes Error-Handling nötig.
    """
...
def get_combined_universe(force_refresh: bool = False) -> Tuple[Set[str], str]:
    """
    Kombiniert Watchlist mit aktuellen Top Movers.
    
    CHANGELOG: 2026-03-08 - Cache-Logik verhindert zu häufige API-Calls.
    """
...
def get_symbol_source(symbol: str) -> SourceType:
    """Bestimmt Herkunft eines Symbols (Watchlist vs Movers)."""
...
def get_finnhub_news_smart(symbol: str) -> Tuple[Optional[List[Dict]], bool]:
    """
    Holt News von Finnhub mit Caching und Rate-Limiting.
    
    CHANGELOG: 2026-03-08 - Rotiert durch FINNHUB_KEYS bei Fehlern.
    """
...
def analyze_news_tiered(symbol: str, tier: int, score: int) -> Tuple[List[Dict], List[str], bool]:
    """
    Tier-basierte News-Analyse. Höhere Tiers = weniger API-Calls.
    
    CHANGELOG: 2026-03-08 - Sparsamer Umgang mit API-Limits.
    """
    if tier <= 20 or score > 60:
        news, cached = get_finnhub_news_smart(symbol)
        if news:
            sources = ['FH']
            bullish_kws = ['bullish', 'upgrade', 'beat', 'growth', 'positive', 'strong']
...
def analyze_candlestick(df: pd.DataFrame, swing_low: float, recent_high: float) -> CandlestickSignal:
    """
    Kern-Engine für Candlestick-Pattern-Erkennung.
    
    CHANGELOG: 2026-03-08 - Implementiert 7 Muster mit quantitativer Stärke-Bewertung.
    Jedes Muster hat spezifische geometrische Bedingungen (Schatten/Body-Verhältnisse).
    
    DESIGN-ENTSCHEIDUNG: Harte Filterung (strength >= 60) eliminiert False Positives
    in sideways Märkten, kann aber in stark trendenden Märkten zu konservativ sein.
    """
    if len(df) < 5:
    ...
    in_pullback = 0.05 < dist_from_high < 0.50
    ...
    # PATTERN 4: MORNING STAR (3-Kerzen)
    is_morning_star = (
        p1['bearish'] and
...
def analyze_structure(df: Optional[pd.DataFrame], symbol: Optional[str] = None) -> Dict[str, Any]:
    """
    Identifiziert Swing Highs/Lows und Trend-Struktur (HH/HL).
    
    CHANGELOG: 2026-03-08 - Verwendet lokale Extrema (5-Punkt-Methodik).
    """
...
def get_alpha_vantage_smart(symbol: str) -> Tuple[Optional[Dict], bool]:
    """
    Fundamentale Daten mit Key-Rotation und Caching.
    
    CHANGELOG: 2026-03-08 - Überspringt erschöpfte Keys automatisch.
    """
...
def get_gemini_entry_analysis(item_data: dict) -> str:
    """
    LLM-basierte Entry-Analyse mit Candlestick-Kontext.
    
    CHANGELOG: 2026-03-08 - Integriert Candlestick-Signal in Prompt für
    kohärente technische Analyse.
    """
...
class ThreadPoolBullScanner:
    """
    Haupt-Scanner mit paralleler Verarbeitung und Hard Filter.
    
    CHANGELOG: 2026-03-08 - Thread-sicher durch Locks und Session Context.
    Jedes Symbol durchläuft: Datenabruf → Struktur → Candlestick → Scoring → Filter.
    """
    ...
    def _fetch_yahoo_with_rate_limit(self, symbol: str) -> Optional[pd.DataFrame]:
        """
        Yahoo-Datenabruf mit Rate Limiting und Caching.
        
        CHANGELOG: 2026-03-08 - Exponential Backoff bei Fehlern.
        """
    ...
    def analyze_single_symbol(self, symbol: str, tier: int, total: int) -> Optional[Dict[str, Any]]:
        """
        Vollständige Analyse-Pipeline für ein Symbol.
        
        CHANGELOG: 2026-03-08 - Harte Filterung nach Candlestick-Analyse.
        Debug-Info wird in Session State gespeichert für Transparenz.
        """
        debug_info = {'symbol': symbol, 'tier': tier, 'errors': [], 'checks': {}}
        
        # Rate Limiting für höhere Tiers
        if tier > 30:
            time.sleep(0.5)
        if tier > 60:
            time.sleep(1.0)
        ...
        # 4. CANDLESTICK-ANALYSE (HARD FILTER)
        ...
        if hard_filter_active:
            if candlestick.strength < MIN_CANDLESTICK_STRENGTH:
                debug_info['errors'].append(
                    ...
                    f"(min: {MIN_CANDLESTICK_STRENGTH})"
                )
                _log_scan_debug(debug_info)
                return None
        ...
        # 5. SCORING (nur wenn Candlestick bestanden)
        score = 25  # Basis
    ...
    def scan_batch(self, symbols: List[Tuple[str, SourceType]], progress_callback=None) -> List[Dict]:
        """
        Parallele Verarbeitung der Symbol-Liste.
        
        CHANGELOG: 2026-03-08 - ThreadPool mit Fortschritts-Callback.
        """
        results = []
        completed = 0
        error_count = 0
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_symbol):
                ...
                if progress_callback:
                    progress_callback(
                        completed, len(symbols), success_count, 
                        error_count, candlestick_filtered, symbol, source
                    )
...
def should_send_alert(symbol: str, current_price: float, current_score: int) -> bool:
    """
    Cooldown-Logik für Alerts (Anti-Spam).
    
    CHANGELOG: 2026-03-08 - 60 Minuten Cooldown oder 2% Preisänderung.
    """
...
def record_alert(symbol: str, price: float, score: int, setup_type: str):
    """Speichert Alert in Session State."""
...
def send_telegram_alert(
    ...
    """
    Sendet formatierten Alert an Telegram.
    
    CHANGELOG: 2026-03-08 - Integriert Candlestick-Info in Nachricht.
    """
...
def render_card_html(item: Dict) -> str:
    """Generiert HTML für Ergebnis-Karte."""
    sym = item['symbol']
    price = item['price']
...
def render_card(item: Dict, container):
    """Rendert Karte mit Gemini-Button."""
    ...
    with container:
        ...
        if st.button(f"🤖 Gemini Check", key=unique_btn_key):
            with st.spinner(f"Analysiere {item['symbol']}..."):
...
def main():
    ...
    with st.sidebar:
        ...
        auto_pilot = st.toggle(
            ...
            help="Scannt automatisch alle 60 Minuten"
        )
        ...
        hard_filter = st.toggle(
            "🔥 Hard Filter (Nur Candlestick-Setups)", 
        ...
        stats = st.session_state.get('api_stats', {
            ...
            'cache_hits':0,'alpha_rotation_count':0
        })
        ...
        if st.button("🔄 Stats zurücksetzen"):
            ...
            st.session_state['combined_universe'] = set(
                DEFAULT_WATCHLIST + [s for sublist in FALLBACK_MOVERS.values() for s in sublist]
            )
                ...
                if item['score'] > 75:
                    ...
                    if should_send_alert(symbol, price, score):
                        ...
                        if success:
                            ...
                            alerts_sent += 1
                            if alerts_sent <= 3:
                                st.toast(f"🚨 {setup_type} Alert: {symbol} @ ${price:.2f}")
    ...
    if st.session_state.get('auto_refresh', False):
        last_scan = st.session_state.get('last_scan_time')
        if last_scan:
            next_scan = last_scan + timedelta(minutes=60)
